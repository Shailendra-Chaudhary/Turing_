...
// Mock functions for output (LED, buzzer, display)
...
int main() {
    ...
    if (error != SUCCESS) {
        ...
        displayMessage("Sensor Value: ");
        displayMessage(std::to_string(sensorValue).c_str());
    }

    // Attempt to send data over the network
    char dataToSend[] = "Test Data";
    error = sendDataOverNetwork(dataToSend);
